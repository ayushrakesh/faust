...
static Tree realeval(Tree exp, Tree visited, Tree localValEnv)
{
    ...
    if (xt || isBoxInt(exp) || isBoxReal(exp) || isBoxWire(exp) || isBoxCut(exp) || isBoxPrim0(exp) ||
    ...
    } else if (isBoxModulation(exp, label, body)) {
        // Evaluation of a box modulation expression that need to rewrite the body
        // Evaluate the label
        string s1     = evalLabel(tree2str(label), visited, localValEnv);
        Tree   elabel = tree(s1.c_str());

        // Create a new slot named after the evaluated label
        Tree         slot = boxSlot(++gGlobal->gBoxSlotNumber);
        stringstream s;
        s << *elabel;
        setDefNameProperty(slot, s.str());
        ...
        Tree result = boxSymbolic(slot, mbody);
        return result;
